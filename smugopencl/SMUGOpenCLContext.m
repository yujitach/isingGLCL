...
- (id)initSimpleContextWithDeviceType:(SMUGOpenCLDeviceType)inDeviceType;
{
    ...
    if ( err != CL_SUCCESS ) {
        ERROR( @"Error retrieving OpenCL device ID for type %d", (int)mDeviceType );
        return nil;
    }
	...
	//
	context = clCreateContext(akProperties, 0, 0, NULL, NULL, &err);
    }else{
	context = clCreateContext( NULL, 1, &mDeviceID, NULL, NULL, &err );
    }
    if ( err != CL_SUCCESS ) {
        ERROR( @"Error retrieving OpenCL device ID for type %d", (int)mDeviceType );
        return nil;
    ...
    if ( err != CL_SUCCESS ) {
        ERROR( @"Could not create command queue for device %d", (int)mDeviceID );
...
- (id)initGPUContextForGLContext:(NSOpenGLContext*)c;
{
    glContext=[c retain];
    return [self initSimpleContextWithDeviceType:CL_DEVICE_TYPE_GPU];
}
...
- (size_t)workgroupSizeForKernel:(SMUGOpenCLKernel*)kernel
{
    ...
    cl_int err = clGetKernelWorkGroupInfo( 
        ...
        NULL );
    if ( err != CL_SUCCESS ) {
        ERROR( @"Could not get work group size for device %d", (int)mDeviceID );
        return 0;
    }
